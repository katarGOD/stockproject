...
export default {
  ...
  methods: {
    ...
    getEmployeeSortByName () {
      return new Promise(resolve => {
        ...
        vm.$database.collection('department')
          ...
          .then(function (docs) {
            datatable.push([
              vm.$t('Index'), vm.$t('Code'),
              vm.$t('Description')
            ])
            docs.forEach(function (doc) {
              deptCount++
              datatable.push([
                {text: `${doc.data().index}`, alignment: 'left'},
                {text: doc.data().code, alignment: 'left'},
                {text: doc.data().description, alignment: 'left'}
              ])
            })
            datatable.push([
              {text: 'Total Department', fontSize: 16, bold: true, alignment: 'left'},
              {text: deptCount, alignment: 'right', fontSize: 16, bold: true, colSpan: 2}
            ])
            result.push(
              {
    ...
    async printReport () {
      ...
      let reportData = []
      vm.$q.loading.show()
      reportData = await vm.getEmployeeSortByName()
      vm.$q.loading.hide()
