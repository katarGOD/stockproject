    ...
    >
      ...
      <q-td slot="body-cell-index" slot-scope="props" :props="props">
        <span small color="secondary">
          <q-btn size="xs" small flat round icon="remove" @click="decreaseIndex(props, $firestore.authGroup)" />
          {{ props.value }}
          <q-btn size="xs" small flat round icon="add" @click="increaseIndex(props, $firestore.authGroup)" />
        </span>
      </q-td>
      <!-- code -->
      <q-td slot="body-cell-code" slot-scope="props" :props="props">
        <q-btn
          flat
          :label="props.value"
          @click="selected=[_.find(authGroup, {'.key': props.row['.key']})]; openUpdateFrom(inputForm, selected[0])"
        />
      </q-td>
    </q-table>
    <!-- ///////// -->
        ...
        <div class="col-xl-8 col-lg-8 col-md-8 col-sm12 col-xs-12">
          ...
          >
            <q-select
              ...
              toggle
              align="left"
              :display-value="`${inputForm.permission.length} ${$t('selected')}`"
              color="secondary"
...
import authPermissionOptions from 'src/components/options/authPermissionOptions'
...
export default {
  ...
  firestore () {
    return {
      authGroup: this.$database.collection('authGroup'),
  ...
  methods: {
    async addForm () {
      ...
      result = await vm.addProcess(vm.$firestore.authGroup, vm.inputForm, vm.$v.inputForm)
      console.log(result)
    },
    async updateForm () {
      let vm = this
      ...
      await vm.updateProcess(vm.$firestore.authGroup, vm.inputForm, vm.$v.inputForm)
    },
    async deleteRow () {
      let vm = this
      await vm.deleteProcess(vm.selected, vm.$firestore.authGroup)
      vm.selected = []
    },
    createExportData () {
      let vm = this
      vm.$firestore.authGroup
        .orderBy('code')
        .get()
        .then(function (docs) {
  ...
  created: async function () {
    [this.collectionSize, this.loading] = await this.loadingState(this.$firestore.authGroup)
