<template>
  <q-page>
    ...
    >
      ...
      <q-td slot="body-cell-authUserName" slot-scope="props" :props="props">
        <q-btn
          flat
          :label="props.value"
...
export default {
  ...
  data () {
    return {
      ...
      leaveRequestColumns: [
        ...
        { name: 'authUserName', label: this.$t('Name'), field: 'authUserName', sortable: true, align: 'left' },
      ...
      leaveRequestVisibleColumns: ['absenceType', 'authUserName', 'datesRequestedFrom', 'datesRequestedTo', 'approval'],
  ...
  methods: {
    ...
    async iOpenUpdateFrom () {
      ...
      vm.inputForm.approvedBy = vm.userId
  ...
  watch: {
    approvalStatus () {
      if (this.approvalStatus === 'all') {
        this.$binding('leaveRequestDatatable', this.$database.collection('leaveRequest')
          .where('calendarYear', '==', this.filterByYear)
          .orderBy('datesRequestedFrom', 'desc'))
      } else if (this.approvalStatus === 'approval') {
        this.$binding('leaveRequestDatatable', this.$database.collection('leaveRequest')
          .where('calendarYear', '==', this.filterByYear)
          .where('approval', '==', true)
          .orderBy('datesRequestedFrom', 'desc'))
      } else if (this.approvalStatus === 'waiting') {
        this.$binding('leaveRequestDatatable', this.$database.collection('leaveRequest')
          .where('calendarYear', '==', this.filterByYear)
          .where('approval', '==', false)
          .orderBy('datesRequestedFrom', 'desc'))
