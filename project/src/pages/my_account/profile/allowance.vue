    ...
    >
      ...
      <q-td slot="body-cell-accountCode" slot-scope="props" :props="props">
        <q-btn
          ...
          @click="selected=[_.find(allowanceTb, {'.key': props.row['.key']})]; openUpdateFrom(inputForm, selected[0])"
        />
      </q-td>
      <!-- custom tax -->
        ...
        <div class="col-xl-8 col-lg-8 col-md-8 col-sm12 col-xs-12">
          ...
          >
            <q-select
              ...
              :disable="!hasPermission(['personnel--employee-management--update'])"
              :options="allowanceAccountCodeOptions"
...
export default {
  ...
  data () {
    return {
    // app variable
      subMenu: this.$t('Allowance'),
      allowanceAccountCodeOptions: [],
      // datatable
      pagination: {
        sortBy: 'accountCode',
        descending: false
      ...
      columns: [
        ...
        { name: 'quantity', label: this.$t('Quantity'), field: 'quantity', sortable: false, align: 'right' },
        { name: 'amount', label: this.$t('Amount'), field: 'amount', sortable: false, align: 'right' },
        { name: 'total', label: this.$t('Total'), field: 'total', sortable: false, align: 'right' }
      ],
  ...
  firestore () {
    return {
      allowanceTb: this.$database.collection('allowanceDetail').where('authUser', '==', this.$route.params.id),
      allowance: this.$database.collection('allowanceDetail')
    }
  },
  // validations
  validations: {
  ...
  methods: {
    async addForm () {
      ...
      await vm.addProcess(vm.$firestore.allowance, vm.inputForm, vm.$v.inputForm)
    },
    async updateForm () {
      let vm = this
      vm.inputForm.postingCategory = vm.postingCategory
      ...
      await vm.updateProcess(vm.$firestore.allowance, vm.inputForm, vm.$v.inputForm)
    },
    async deleteRow () {
      let vm = this
      await vm.deleteProcess(vm.selected, vm.$firestore.allowance)
      vm.selected = []
    },
    ...
    getAllowanceAccountCodeOptions (postingCategory) {
      return new Promise(resolve => {
        let vm = this
        let result = []
        vm.$database.collection('accountCode')
          .where('postingCategory', '==', postingCategory)
          .orderBy('code')
          .get()
          .then(function (docs) {
            docs.forEach(function (doc) {
  ...
  created: async function () {
    [this.collectionSize, this.loading] = await this.loadingState(this.$firestore.allowance)
    let vm = this
    vm.$emit('updateSubMenu', vm.subMenu)
    vm.postingCategory = await vm.getPostingCategory('ALLOW')
    vm.allowanceAccountCodeOptions = await vm.getAllowanceAccountCodeOptions(vm.postingCategory)
  },
  // computed
  computed: {
    ...mapGetters([
